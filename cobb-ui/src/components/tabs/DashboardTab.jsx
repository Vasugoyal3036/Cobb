    ...
    <>
      {activeTab === 'dashboard' && (
        <div className="space-y-6">
                ...
                {/* Cobb AI Copilot Dashboard Tile */}
                <div className={`relative overflow-hidden rounded-2xl p-5 sm:p-6 border shadow-sm transition-all hover:shadow-md ${
                  darkMode
                    ? 'bg-gradient-to-r from-slate-900 via-indigo-950/60 to-slate-900 border-indigo-900/40 text-slate-100'
                    : 'bg-gradient-to-r from-blue-50 via-indigo-50/70 to-purple-50 border-indigo-200/70 text-slate-800'
                }`}>
                  {/* Decorative ambient background icon */}
                  <div className="absolute -right-6 -bottom-6 opacity-10 dark:opacity-20 pointer-events-none">
                  ...
                  <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                    <div className="space-y-1.5 max-w-xl">
                      <div className="flex items-center gap-2">
                        <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-xs">
                          <Sparkles className="w-3 h-3 animate-pulse" />
                          Cobb AI Copilot
                        </span>
                        <span className="text-[11px] font-bold text-indigo-600 dark:text-indigo-400">
                          Live Store Database Intelligence
                        </span>
                      </div>
                      <h3 className="text-lg sm:text-xl font-black tracking-tight">
                        Ask Cobb AI Anything
                      </h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                        Natural language queries for live inventory, size breakdown, today's sales, UPI split, and dead stock.
                      </p>
                    </div>

                    {/* Quick Query Chips & Action Button */}
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 shrink-0">
                      <div className="flex flex-wrap items-center gap-1.5">
                        <button
                          type="button"
                          onClick={() => setActiveTab('copilot')}
                          className={`text-xs px-3 py-1.5 rounded-xl font-semibold transition-all hover:scale-102 active:scale-98 cursor-pointer ${
                            darkMode
                              ? 'bg-slate-800/90 hover:bg-slate-800 text-indigo-300 border border-slate-700'
                              : 'bg-white hover:bg-slate-50 text-indigo-700 border border-indigo-200 shadow-xs'
                          }`}
                        >
                          👔 Full Sleeves Stock
                        </button>
                      ...
                      <button
                        type="button"
                        onClick={() => setActiveTab('copilot')}
                        className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 text-white font-bold text-xs shadow-md hover:shadow-indigo-500/30 hover:scale-105 active:scale-95 transition-all cursor-pointer shrink-0"
                      >
