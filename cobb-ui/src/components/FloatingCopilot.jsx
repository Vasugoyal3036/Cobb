...
export default function FloatingCopilot({
  ...
  const [isPhoneOrTunnel, setIsPhoneOrTunnel] = useState(() => {
    ...
    return (isMobileDevice && isMobileWidth) || isPhoneQuery;
  });
  ...
  useEffect(() => {
    const checkPhone = () => {
      ...
      const isPhoneQuery = new URLSearchParams(window.location.search).has('phone') || 
                           new URLSearchParams(window.location.search).has('mobile');
      setIsPhoneOrTunnel((isMobileDevice && isMobileWidth) || isPhoneQuery);
    };
  ...
  const handleSaveApiKey = async () => {
    ...
    try {
      ...
      if (data.success) {
        setKeyFeedback({ success: true, message: data.message || 'Gemini AI connected!' });
        refreshEngineStatus();
  ...
  const executeAction = (action) => {
    ...
    } else if (action.type === 'NAVIGATE' || action.type === 'NAVIGATE_TAB') {
      const target = action.target || action.tab;
      if (onNavigateTab) {
        onNavigateTab(target);
      } else {
        window.dispatchEvent(new CustomEvent('switchTab', { detail: target }));
      }
      setIsOpen(false);
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={`flex fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-[460px] h-[620px] max-h-[85vh] rounded-3xl shadow-2xl flex-col overflow-hidden border transition-all animate-in fade-in slide-in-from-bottom-5 duration-200 ${
        ...
        >
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                <h3 className="text-sm font-bold flex items-center space-x-1.5">
                  ...
                  <span className={`w-2 h-2 rounded-full ${engineStatus.geminiActive ? 'bg-purple-300 animate-pulse' : 'bg-emerald-400'}`} />
                  <span className="text-[10px] font-normal px-1.5 py-0.2 rounded-full bg-white/20">
                    {engineStatus.geminiActive ? (engineStatus.model || 'Gemini 3.6') : 'Local'}
                  </span>
                </h3>
                <p className="text-[10px] text-blue-100">Live POS, Inventory & Marketing Intelligence</p>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-1">
              <button
                ...
                className={`p-1.5 rounded-lg transition-colors cursor-pointer ${
                  engineStatus.geminiActive ? 'bg-purple-500/40 hover:bg-purple-500/60' : 'hover:bg-white/20'
                } text-white`}
          ...
          >
            <button
              type="button"
              onClick={() => handleSend("buy 3 get 70% on mrp 2499")}
            ...
            <button
              type="button"
              onClick={() => handleSend("what items are currently on hold")}
              className="shrink-0 px-2.5 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-300 font-bold text-[10.5px] border border-indigo-200 dark:border-indigo-800 hover:scale-102 transition-transform cursor-pointer"
            >
              ⏳ Holds Desk
            </button>
            <button
              type="button"
              onClick={() => handleSend("draft whatsapp promo message for buy 3 get 70%")}
            ...
            <button
              ...
              className="shrink-0 px-2.5 py-1 rounded-full bg-violet-50 dark:bg-violet-950/80 text-violet-600 dark:text-violet-300 font-bold text-[10.5px] border border-violet-200 dark:border-violet-800 hover:scale-102 transition-transform cursor-pointer"
            >
              🚚 In Transit
            </button>
          </div>
                      ...
                      <div className="mt-2.5 pt-2 border-t border-slate-700/50 flex flex-wrap gap-1.5">
                        {m.actions.map((act, actIdx) => (
                          <button
                            ...
                            className={`text-[10px] font-bold px-2.5 py-1 rounded-lg text-white flex items-center gap-1 shadow-xs transition-colors cursor-pointer hover:scale-102 ${
                              act.type === 'LINK'
          ...
          {/* Gemini Key Setup Modal inside Drawer */}
          {showKeyModal && (
            <div className="absolute inset-0 z-50 bg-slate-950/90 backdrop-blur-xs p-5 flex flex-col justify-between animate-in fade-in duration-150">
              <div>
                <div className="flex items-center justify-between pb-2 border-b border-slate-800">
                  <div className="flex items-center space-x-2">
                    <Sparkles className="w-4 h-4 text-purple-400" />
                    <h4 className="text-sm font-bold text-white">Google Gemini AI Setup</h4>
                  </div>
                  <button
                    type="button"
                ...
                <div className="mt-3 p-2.5 rounded-xl bg-purple-500/10 border border-purple-500/30 text-purple-300 text-xs">
                  <p className="font-semibold">{engineStatus.geminiActive ? `✅ ${engineStatus.model || 'Gemini 3.6'} Connected` : '⚡ Local Retail Engine'}</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">
                    {engineStatus.geminiActive
                      ? 'Ask ANY question in plain English. Gemini translates your query to safe SQL directly on your database.'
                      : 'Enter a free Google AI Studio key to ask ANY custom question.'}
                  </p>
                </div>
                ...
                <div className="mt-3">
                  ...
                  <input
                    ...
                    placeholder="AIzaSy..."
                    className="w-full px-3 py-2 rounded-xl font-mono text-xs bg-slate-800 border border-slate-700 text-white outline-none focus:border-purple-500"
                  />
                </div>

                {keyFeedback && (
                  <div className={`mt-2.5 p-2 rounded-xl text-[11px] font-medium border flex items-center space-x-1.5 ${
                    keyFeedback.success ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-300' : 'bg-rose-500/15 border-rose-500/30 text-rose-300'
                  }`}>
                    {keyFeedback.success ? <Check className="w-3.5 h-3.5 shrink-0" /> : <AlertTriangle className="w-3.5 h-3.5 shrink-0" />}
                ...
                <div className="mt-3 text-[11px] space-y-1 text-slate-400">
                  ...
                  >
                    ...
                    <span>Get Free Gemini Key at Google AI Studio (15s)</span>
                  </a>
                </div>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className={`px-3.5 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 text-white cursor-pointer ${
                  ...
                  >
                    ...
                    <span>{savingKey ? 'Validating...' : 'Save Key'}</span>
                  </button>
                </div>
              </div>
            </div>
