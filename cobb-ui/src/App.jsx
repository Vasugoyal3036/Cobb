...
export default function App() {
  ...
  return (
    <div className={`flex h-screen font-sans transition-colors duration-300 ${darkMode ? 'dark-mode bg-slate-950 text-slate-100' : 'bg-slate-100 text-slate-800'}`}>
      ...
      {/* Sidebar Navigation */}
      <div className="w-64 bg-slate-950 text-slate-300 flex flex-col shadow-xl z-20 border-r border-slate-800">
      ...
      <div className="flex-1 overflow-auto relative bg-slate-50">
        ...
        <div className="bg-white/80 backdrop-blur-md px-8 py-4 border-b border-slate-200 flex justify-between items-center sticky top-0 z-10">
          <div className="relative w-full max-w-lg">
            ...
            <input 
              type="text" 
              placeholder="Search Article No, Phone, or Client Name (Press Enter)" 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onKeyDown={handleGlobalSearch}
              className="w-full pl-9 pr-4 py-2.5 bg-slate-100 border-none rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all shadow-inner"
            />
          </div>
          <div className="flex items-center space-x-3">
             ...
             <button 
               ...
               className="dark-toggle-btn p-2 bg-slate-100 hover:bg-slate-200/80 border border-slate-200 text-slate-650 rounded-xl transition-all shadow-sm cursor-pointer flex items-center justify-center"
             ...
             <div className="status-badge flex items-center space-x-2 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 shadow-sm cursor-help" title="Automation Engine Status">
               <div className="relative flex h-2.5 w-2.5">
                  ...
                  <span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${isGatewayRunning && isListenerRunning ? 'bg-green-500' : 'bg-red-500'}`}></span>
               </div>
               <span className="text-xs font-bold text-slate-600 uppercase tracking-wide">
                 {isGatewayRunning && isListenerRunning ? 'Systems Active' : 'Systems Offline'}
               </span>
             </div>
          </div>
        </div>
