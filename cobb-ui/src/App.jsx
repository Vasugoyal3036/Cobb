...
export default function App() {
  ...
  const [broadcastMedia, setBroadcastMedia] = useState(null);
  const [broadcastMediaPath, setBroadcastMediaPath] = useState(null);
  ...
  const handleBroadcastMediaUpload = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/api/broadcast/upload`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      setBroadcastMedia({ name: file.name, size: file.size, type: file.type });
      setBroadcastMediaPath(res.data.filePath);
    } catch (err) {
  ...
  const handleRemoveBroadcastMedia = () => {
    setBroadcastMedia(null);
    setBroadcastMediaPath(null);
  };
              ...
              <div className="p-4 sm:p-6 lg:p-8 space-y-8">
                ...
                <div className="bg-white p-4 sm:p-6 lg:p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
                  ...
                  <div className="flex items-center gap-3">
                    <label className="flex items-center gap-2 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold cursor-pointer transition-all border border-slate-200">
                      <Paperclip className="w-4 h-4" />
                      {isUploadingMedia ? 'Uploading...' : 'Attach Media'}
                      <input type="file" accept="image/*,video/*,.pdf,.doc,.docx" className="hidden" onChange={handleBroadcastMediaUpload} disabled={isUploadingMedia} />
                    </label>
                    {broadcastMedia && (
                      <div className="flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-200 rounded-xl text-xs">
                        <Image className="w-4 h-4 text-emerald-600" />
                        <span className="font-bold text-emerald-700 max-w-[200px] truncate">{broadcastMedia.name}</span>
                        <span className="text-emerald-500">({(broadcastMedia.size / 1024).toFixed(0)} KB)</span>
                        <button onClick={handleRemoveBroadcastMedia} className="ml-1 text-red-400 hover:text-red-600 cursor-pointer"><X className="w-4 h-4" /></button>
                      </div>
                    )}
                  </div>
