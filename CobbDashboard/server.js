...
app.get('/api/music/search', async (req, res) => {
    ...
    if (ytMusicCache.has(cacheKey)) {
        return res.json({ results: ytMusicCache.get(cacheKey) });
    }
    ...
    try {
        ...
        ytMusicCache.set(cacheKey, results);
        if (ytMusicCache.size > 200) {
            const firstKey = ytMusicCache.keys().next().value;
            ytMusicCache.delete(firstKey);
        }
